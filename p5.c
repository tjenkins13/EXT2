...
int main(int argc,char **argv)
{
   ...
   while(dir.inode!=0 && block<=(in.i_blocks-1)){
     ...
     lseek(fd,(K*in.i_block[block])+pos,SEEK_SET); //go to block in inode
     rv=read(fd,&dir,sizeof(DIR_STRUCT)-255); //read in directory without name
   
     if(rv!=sizeof(DIR_STRUCT)-255){ 
       fprintf(stderr,"Bad directory read\n");
       exit(0);
     }
     read(fd,&dir.name,dir.name_len); //read in name based on dir.name_len
     dir.name[dir.name_len]=0;

     if(dir.inode==0)
       break;
     
     lseek(fd,(K*bg.bg_inode_table)+((dir.inode-1)*sizeof(INODE)),SEEK_SET); //go to where directory entry's inode is

     rv=read(fd,&in2,sizeof(INODE)); //read in inode
     if(rv!=sizeof(INODE)){
       fprintf(stderr,"Bad Inode read\n");
       exit(0);
     }
